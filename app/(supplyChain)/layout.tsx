...
function LayoutContent({ children }: {
    ...
    return (<OfflineDetector autoReconnect={true} reconnectInterval={30000} blurAmount={4}>
      <CustomCursor containerRef={containerRef}/>
      <div ref={containerRef} className="supplychain-container font-rethink bg-[#FCFBF9] dark:bg-ink min-h-screen">
        <AnimatePresence mode="wait">
          {!isAIOpen && (<motion.div initial={{ y: 0, opacity: 1 }} exit={{ y: -100, opacity: 0 }} transition={{
                duration: 0.3,
                ease: "easeInOut"
            }}>
              <AceternityNavbar />
...
export default function SupplyChainLayout({ children, }: {
    ...
    return (<SessionGuard requiredRole={['Admin', 'Manager', 'Employee', 'Operator', 'Executive']}>
      <AIProvider>
