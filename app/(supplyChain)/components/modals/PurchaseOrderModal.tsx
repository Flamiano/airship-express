...
export function PurchaseOrderModal({ isOpen, onClose, request, onOrderCreated, }: PurchaseOrderModalProps) {
    ...
    const [formData, setFormData] = useState({
        delivery_date: "",
        notes: "",
        items: [] as Array<PurchaseRequestItem & {
            unit_price: number;
            total: number;
        }>,
    });
    ...
    return (<div className="fixed inset-0 bg-slate-950/60 dark:bg-slate-950/70 backdrop-blur-md flex items-center justify-center z-50 p-4 animate-in fade-in duration-200" onClick={onClose}>
            <div className="bg-white dark:bg-slate-900 rounded-2xl max-w-2xl w-full max-h-[90vh] overflow-y-auto p-6 shadow-2xl dark:shadow-black/60 border border-slate-100 dark:border-white/10 animate-in zoom-in-95 duration-200" onClick={(e) => e.stopPropagation()}>
                ...
                {step === 1 && (<div className="space-y-5 animate-in slide-in-from-right-4 duration-300">
                        <div className="bg-slate-50/80 dark:bg-slate-800/40 rounded-xl p-4 border border-slate-200/80 dark:border-white/10">
                            <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 text-sm">
                                <div>
                                    ...
                                    {supplierEmail && (<p className="text-[10px] text-slate-400 dark:text-slate-500 mt-0.5 truncate">
                                            📧 {supplierEmail}
                                        </p>)}
                                    {supplierMessenger && (<p className="text-[10px] text-sky-400 dark:text-sky-500 mt-0.5 truncate">
                                            💬 Messenger available
                                        </p>)}
                                </div>
                                <div>
                        ...
                        <div>
                            ...
                            <div className="space-y-2 max-h-[280px] overflow-y-auto pr-1">
                                {formData.items.map((item, index) => (<div key={index} className="flex items-center gap-3 bg-white dark:bg-slate-800/60 p-3 rounded-xl border border-slate-200 dark:border-white/10 shadow-xs hover:border-slate-300 dark:hover:border-white/20 transition-all">
                                        ...
                                        <div className="w-36">
                                            <div className="relative rounded-lg shadow-2xs">
                                                ...
                                                <input type="number" min="0" step="0.01" className="w-full bg-slate-50/50 dark:bg-slate-900/50 border border-slate-200 dark:border-white/10 rounded-lg pl-6 pr-2.5 py-1.5 text-sm text-slate-900 dark:text-slate-100 placeholder:text-slate-400 dark:placeholder:text-slate-600 focus:bg-white dark:focus:bg-slate-900 focus:border-emerald-500 focus:ring-2 focus:ring-emerald-500/20 outline-none transition-all" placeholder="0.00" value={item.unit_price || ""} onChange={(e) => updateItem(index, e.target.value)} required/>
                                            </div>
                                        </div>
                        ...
                        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                            <div>
                                ...
                                <input type="date" className="w-full bg-slate-50 dark:bg-slate-800/50 border border-slate-200 dark:border-white/10 rounded-xl px-3.5 py-2.5 text-sm text-slate-900 dark:text-slate-100 focus:bg-white dark:focus:bg-slate-900 focus:border-emerald-500 focus:ring-2 focus:ring-emerald-500/20 outline-none transition-all cursor-pointer" value={formData.delivery_date} min={new Date().toISOString().split("T")[0]} onChange={(e) => setFormData((prev) => ({
                ...prev,
                delivery_date: e.target.value,
                        ...
                        <div>
                            ...
                            <textarea className="w-full bg-slate-50 dark:bg-slate-800/50 border border-slate-200 dark:border-white/10 rounded-xl px-3.5 py-2.5 text-sm text-slate-900 dark:text-slate-100 placeholder:text-slate-400 dark:placeholder:text-slate-600 focus:bg-white dark:focus:bg-slate-900 focus:border-emerald-500 focus:ring-2 focus:ring-emerald-500/20 outline-none transition-all resize-none" rows={2} placeholder="Add any specific instructions or details for this purchase order..." value={formData.notes} onChange={(e) => setFormData((prev) => ({ ...prev, notes: e.target.value }))}/>
                        </div>
                        ...
                        <div className="bg-gradient-to-br from-indigo-50/60 via-purple-50/40 to-slate-50 dark:from-indigo-950/40 dark:via-purple-950/20 dark:to-slate-900 border border-indigo-100 dark:border-indigo-800/30 rounded-xl p-4 sm:p-5 shadow-xs">
                            <div className="flex items-center justify-between mb-3">
                                ...
                                <div className="flex items-center gap-2">
                                    ...
                                    {aiMessage && (<AppButton type="button" variant="neutral" size="xs" onClick={handleCopyOnly} disabled={isSending}>
                            ...
                            <div className="bg-white/90 dark:bg-slate-900/80 backdrop-blur-sm rounded-xl p-4 border border-indigo-100/80 dark:border-indigo-800/20 text-sm text-slate-800 dark:text-slate-200 leading-relaxed shadow-2xs min-h-[120px]">
                                {isGeneratingAI ? (<div className="flex items-center justify-center h-20">
                                    ...
                                    </div>) : (<p className="text-slate-400 dark:text-slate-500 italic text-center py-6">
                                        Click "Generate with AI" to create a professional supplier message
                                    </p>)}
                            </div>

                            <div className="mt-3 grid grid-cols-1 sm:grid-cols-2 gap-2 text-xs">
                                {supplierEmail && (<div className="flex items-center gap-2 text-slate-500 dark:text-slate-400 bg-white/60 dark:bg-slate-800/40 p-2 rounded-lg border border-slate-100 dark:border-white/10">
                                        <i className="fas fa-envelope text-blue-400"/>
                                        <span>Email: <strong>{supplierEmail}</strong></span>
                                    </div>)}
                                {supplierMessenger && (<div className="flex items-center gap-2 text-slate-500 dark:text-slate-400 bg-white/60 dark:bg-slate-800/40 p-2 rounded-lg border border-slate-100 dark:border-white/10">
