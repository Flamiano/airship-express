...
export default function DocumentViewerModal({ isOpen, onClose, data }: DocumentViewerModalProps) {
    ...
    return (<div className="fixed inset-0 z-50 flex items-center justify-center p-3 sm:p-6 bg-slate-950/60 dark:bg-slate-950/70 backdrop-blur-md animate-in fade-in duration-200">
            <div className="relative w-full max-w-5xl h-[92vh] max-h-[900px] bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-3xl shadow-2xl overflow-hidden flex flex-col">
                ...
                <div className="flex items-center justify-between px-5 sm:px-6 py-4 border-b border-slate-200 dark:border-slate-800/80 bg-slate-50/70 dark:bg-slate-900/60 shrink-0">
                    <div className="flex items-center gap-3 min-w-0">
                        ...
                        <div className="min-w-0">
                            <div className="flex items-center gap-2 flex-wrap">
                                ...
                                {data.verifiedStatus === 'matched' && (<StatusBadge tone="pink" icon="fas fa-check-circle" size="xs">
                                        Matched ✓
                                    </StatusBadge>)}
                                {data.verifiedStatus === 'mismatched' && (<StatusBadge tone="amber" icon="fas fa-exclamation-triangle" size="xs">
                                        Mismatch ⚠
                                    </StatusBadge>)}
                                {data.verifiedStatus === 'forced' && (<StatusBadge tone="indigo" icon="fas fa-shield-alt" size="xs">
                                        Admin Forced ✓
                                    </StatusBadge>)}
                            </div>
                    ...
                    <div className="flex items-center gap-1.5 sm:gap-2 shrink-0">
                        {!isPdf && resolvedUrl && (<div className="hidden sm:flex items-center gap-1 bg-slate-100 dark:bg-slate-800 p-1 rounded-xl border border-slate-200 dark:border-slate-700/60">
                                <button type="button" onClick={handleZoomOut} className="p-1.5 text-slate-600 dark:text-slate-300 hover:text-pink-600 dark:hover:text-pink-400 rounded-lg hover:bg-white dark:hover:bg-slate-700 transition-all cursor-pointer" title="Zoom Out">
                                ...
                                <button type="button" onClick={handleZoomIn} className="p-1.5 text-slate-600 dark:text-slate-300 hover:text-pink-600 dark:hover:text-pink-400 rounded-lg hover:bg-white dark:hover:bg-slate-700 transition-all cursor-pointer" title="Zoom In">
                                    <i className="fas fa-search-plus text-xs"/>
                                </button>
                                <button type="button" onClick={handleRotate} className="p-1.5 text-slate-600 dark:text-slate-300 hover:text-pink-600 dark:hover:text-pink-400 rounded-lg hover:bg-white dark:hover:bg-slate-700 transition-all cursor-pointer border-l border-slate-200 dark:border-slate-700 pl-2" title="Rotate 90°">
                                    <i className="fas fa-redo text-xs"/>
                                </button>
                                <button type="button" onClick={handleResetZoom} className="p-1.5 text-slate-500 hover:text-slate-800 dark:text-slate-400 dark:hover:text-white rounded-lg hover:bg-white dark:hover:bg-slate-700 transition-all cursor-pointer text-[10px]" title="Reset">
                                    Reset
                                </button>
                            </div>)}
                ...
                <div className="flex-1 min-h-0 bg-slate-900/95 dark:bg-slate-950 relative overflow-auto flex items-center justify-center p-4">
                    {isLoading && (<div className="flex flex-col items-center justify-center gap-3 text-slate-400 py-12">
                    ...
                    {hasError && !isLoading && (<div className="flex flex-col items-center justify-center gap-2 text-center p-6 max-w-sm">
                            ...
                            {resolvedUrl && (<a href={resolvedUrl} target="_blank" rel="noreferrer" className="mt-3 px-3.5 py-1.5 bg-pink-500 text-white rounded-xl text-xs font-semibold hover:bg-pink-600 transition-all shadow-xs">
                                    Open Link Directly
                                </a>)}
                        </div>)}

                    {!isLoading && !hasError && resolvedUrl && (<EmbeddedDocViewer url={resolvedUrl} fileName={data.fileName} title={data.title} fileType={data.fileType} storagePath={data.storagePath} onDownload={handleDownload} zoom={zoom} rotation={rotation} minHeight="min-h-[460px] sm:min-h-[580px]"/>)}
                </div>
                ...
                <div className="px-6 py-3 border-t border-slate-200 dark:border-slate-800 bg-slate-50 dark:bg-slate-900/90 flex items-center justify-between flex-wrap gap-2 text-xs text-slate-600 dark:text-slate-400 shrink-0">
                    <div className="flex items-center gap-4 flex-wrap">
                        {data.totalAmount !== undefined && (<div>
                        ...
                        {data.notes && (<div className="italic text-[11px] text-slate-500 truncate max-w-md">
                                {data.notes}
                            </div>)}
                    </div>
