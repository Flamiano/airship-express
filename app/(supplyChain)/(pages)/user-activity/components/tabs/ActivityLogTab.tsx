...
export const ActivityLogTab: React.FC<ActivityLogTabProps> = ({ activities, isLoading, searchTerm, onSearchTermChange, filter, onFilterChange, uniqueActions, selectedActivities, onToggleSelectActivity, onSelectAllActivities, onBulkDelete, currentPage, totalPages, onPageChange, }) => {
    ...
    return (<div className="bg-white dark:bg-[#1c1d25] rounded-2xl border border-slate-200/90 dark:border-[#353746] shadow-[0_4px_16px_rgba(0,0,0,0.06)] dark:shadow-[0_4px_20px_rgba(0,0,0,0.5)] overflow-hidden">
            ...
            {/* Bulk Actions Banner */}
            {selectedActivities.size > 0 && (<div className="p-3 bg-pink-50/60 dark:bg-pink-950/30 border-b border-pink-100 dark:border-pink-900/40 flex items-center justify-between flex-wrap gap-2 transition-all animate-in fade-in duration-150">
                    ...
                    <thead>
                        <tr className="border-b border-slate-200/60 dark:border-slate-800 bg-slate-50/70 dark:bg-slate-900/80 text-[11px] font-bold tracking-wider text-slate-400 dark:text-slate-500 uppercase select-none">
                            <th className="py-3 px-4 w-10 text-center">
                                <input type="checkbox" checked={allActivitiesSelected} ref={(input) => {
            if (input) {
                input.indeterminate = someActivitiesSelected;
            }
        }} onChange={onSelectAllActivities} className="w-4 h-4 rounded border-slate-300 dark:border-slate-700 text-pink-500 focus:ring-pink-500/20 cursor-pointer accent-pink-500 bg-transparent"/>
                            </th>
                ...
                { type: 'date' },
                            ...
                            </tr>) : (activities.map((activity, idx) => {
            ...
            const isDanger = activity.action?.includes('FAILED') ||
                activity.action?.includes('ERROR') ||
                activity.action?.includes('BLOCKED');
            const actionTone = isSuccess ? 'emerald' : isDanger ? 'rose' : 'pink';
            return (<tr key={key} className={`transition-colors hover:bg-slate-50/80 dark:hover:bg-slate-800/40 ${isSelected ? 'bg-pink-50/30 dark:bg-pink-950/20' : ''}`}>
                                        <td className="py-3 px-4 text-center">
                                            <input type="checkbox" checked={isSelected} onChange={() => onToggleSelectActivity(activity.id)} className="w-4 h-4 rounded border-slate-300 dark:border-slate-700 text-pink-500 focus:ring-pink-500/20 cursor-pointer accent-pink-500"/>
                                        </td>
