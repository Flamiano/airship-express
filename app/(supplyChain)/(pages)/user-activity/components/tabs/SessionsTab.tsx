...
export const SessionsTab: React.FC<SessionsTabProps> = ({ sessions, isLoading, searchTerm, onSearchTermChange, selectedSessions, onToggleSelectSession, onSelectAllSessions, onBlockDevice, onBulkBlock, onBulkDelete, currentPage, totalPages, onPageChange, }) => {
    ...
    return (<div className="bg-white dark:bg-[#1c1d25] rounded-2xl border border-slate-200/90 dark:border-[#353746] shadow-[0_4px_16px_rgba(0,0,0,0.06)] dark:shadow-[0_4px_20px_rgba(0,0,0,0.5)] overflow-hidden">
            {/* Search Header */}
            <div className="p-4 border-b border-slate-100 dark:border-[#353746] bg-slate-50/70 dark:bg-[#15161c]">
                <div className="relative max-w-md">
                    ...
                    <input type="text" placeholder="Search sessions by user, IP, or user agent..." value={searchTerm} onChange={(e) => onSearchTermChange(e.target.value)} className="w-full pl-9 pr-4 py-2 text-sm bg-white dark:bg-[#2a2a2e] border border-slate-200/90 dark:border-[#353746] rounded-xl text-slate-800 dark:text-slate-200 placeholder:text-slate-400 dark:placeholder:text-slate-500 focus:outline-none focus:ring-2 focus:ring-pink-500/20 focus:border-pink-500 dark:focus:border-pink-500/80 focus:bg-white dark:focus:bg-[#23242e] transition-all shadow-2xs"/>
                </div>
            </div>

            {/* Bulk Actions Banner */}
            {selectedSessions.size > 0 && (<div className="p-3 bg-slate-50/80 dark:bg-slate-800/60 border-b border-slate-100 dark:border-slate-800 flex items-center justify-between flex-wrap gap-2">
                    <span className="text-xs font-medium text-slate-600 dark:text-slate-300">
                        {selectedSessions.size} session(s) selected
                    </span>
                    <div className="flex items-center gap-2 flex-wrap">
                        <AppButton type="button" variant="danger" size="xs" onClick={onBulkBlock}>
                            <Ban className="w-3 h-3"/>
                            <span>Block Selected</span>
                        </AppButton>
                        <AppButton type="button" variant="neutral" size="xs" onClick={onBulkDelete}>
                    ...
                    <thead>
                        <tr className="border-b border-slate-200/60 dark:border-slate-800 bg-slate-50/70 dark:bg-slate-900/80 text-[11px] font-bold tracking-wider text-slate-400 dark:text-slate-500 uppercase select-none">
                            <th className="py-3 px-4 w-10 text-center">
                                <input type="checkbox" checked={allSessionsSelected} ref={(input) => {
            if (input) {
                input.indeterminate = someSessionsSelected;
            }
        }} onChange={onSelectAllSessions} className="w-4 h-4 rounded border-slate-300 dark:border-slate-700 text-pink-500 focus:ring-pink-500/20 cursor-pointer accent-pink-500 bg-transparent"/>
                            </th>
            ...
            return (<tr key={session.id} className={`transition-colors hover:bg-slate-50/80 dark:hover:bg-slate-800/40 ${isSelected
                    ? 'bg-pink-50/30 dark:bg-pink-950/20'
                    : ''} ${isBlocked
                    ? 'opacity-60 bg-red-50/20 dark:bg-red-950/10'
                    : ''}`}>
                                        <td className="py-3 px-4 text-center">
                                            <input type="checkbox" checked={isSelected} disabled={isDisabled} onChange={() => onToggleSelectSession(session.id, isDisabled)} className={`w-4 h-4 rounded border-slate-300 dark:border-slate-700 text-pink-500 focus:ring-pink-500/20 accent-pink-500 bg-transparent ${isDisabled ? 'opacity-30 cursor-not-allowed' : 'cursor-pointer'}`}/>
                                        </td>
                                        ...
                                        <td className="py-3 px-4">
                                            <StatusBadge tone={isBlocked ? 'rose' : session.is_active ? 'emerald' : 'neutral'} dot size="xs">
                                        ...
                                        <td className="py-3 px-4 text-right whitespace-nowrap w-[80px] min-w-[80px]">
                                            <div className="flex items-center justify-end gap-2.5">
                                                {isBlocked ? (<span className="text-xs text-slate-400 dark:text-slate-500 italic px-2" title={`Blocked for ${session.email || session.users?.email}`}>
                                                        Blocked
                                                    </span>) : (<CrudActionButton action="delete" variant="pink" label="Block" title={isAdmin ? 'Cannot block admin users' : 'Block this device'} disabled={isAdmin} onClick={() => onBlockDevice(session.id, session.user_agent, session.ip_address, userName, session.email)}/>)}
                                            </div>
                                        </td>
                                    </tr>);
        }))}
