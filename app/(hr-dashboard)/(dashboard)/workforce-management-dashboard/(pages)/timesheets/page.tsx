  ...
  return (
    <DashboardLayout>
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 bg-paper p-5 rounded-2xl border border-line shadow-sm">
        <div>
          <h1 className="text-xl font-bold text-ink">Electronic Timesheet Workflow</h1>
          <p className="text-xs text-muted mt-1">
            Review hours logged, approve overtime, and sync data to payroll.
          </p>
        </div>
        {pendingCount > 0 && (
          <span className="text-xs bg-accent/10 text-accent px-3 py-1 rounded-full font-semibold border border-accent/20">
      ...
      <Card className="p-5 space-y-4">
        <Table>
          ...
          <TBody>
            {timesheets.map((ts) => (
              <TR key={ts.id}>
                <TD className="font-mono font-medium text-accent">{ts.id.slice(0, 8)}</TD>
                <TD className="font-semibold text-ink">{ts.employee?.full_name || 'Unknown'}</TD>
                <TD className="text-muted text-xs">
                  {ts.week_start} – {ts.week_end}
                </TD>
                <TD className="text-ink font-mono text-xs">{ts.load_ref || '—'}</TD>
                <TD className="font-semibold text-ink">{ts.total_hours} hrs</TD>
                <TD className="text-rose-500 font-semibold">{ts.overtime_hours} hrs</TD>
        ...
        {timesheets.length === 0 && !error && (
          <p className="text-xs text-muted text-center py-8">No timesheets yet.</p>
        )}
      </Card>
    </DashboardLayout>
