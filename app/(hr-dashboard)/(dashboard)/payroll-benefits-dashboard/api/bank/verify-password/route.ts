...
export async function GET(request: NextRequest) {
  try {
    ...
    return NextResponse.json({
      ...
      remaining: Math.max(0, MAX_ATTEMPTS - (attemptRow?.attempts ?? 0)),
      locked,
      minutesLeft,
      maxAttempts: MAX_ATTEMPTS,
    });
  } catch (error) {
    console.error("GET /bank/verify-password error:", error);
    return NextResponse.json(
...
export async function POST(request: NextRequest) {
  try {
    ...
    console.error("POST /bank/verify-password error:", error);
    return NextResponse.json(
      { error: "Internal server error" },
      { status: 500 }
    );
