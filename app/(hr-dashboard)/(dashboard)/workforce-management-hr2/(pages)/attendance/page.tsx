    ...
    <DashboardLayout realtimeConnected={connected}>
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 bg-white p-5 rounded-2xl border border-pink-100 shadow-sm">
        <div>
          <div className="flex items-center gap-3">
            ...
            <span className={`inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-xs font-bold ${hwOnline ? 'bg-emerald-100 text-emerald-800 border border-emerald-300' : 'bg-amber-100 text-amber-800 border border-amber-300'}`}>
              <Radio size={13} className={hwOnline ? 'animate-pulse text-emerald-600' : ''} />
              {hwOnline ? 'ESP32 Node 01 Online' : 'ESP32 Gateway Offline'}
            </span>
          </div>
          <p className="text-xs text-pink-600 mt-1">Real-time biometric & RFID terminal scanner logs synced to Supabase database.</p>
        </div>
        <div className="flex items-center gap-2">
          <Button onClick={() => { refetch(); loadProfiles(); }} variant="secondary" className="flex items-center gap-1.5 text-xs py-2 px-3 bg-pink-50 text-pink-900 border-pink-200">
        ...
        <div className="fixed inset-0 z-50 bg-pink-950/60 backdrop-blur-sm flex items-center justify-center p-4">
          <div className="bg-white rounded-3xl border border-pink-200 shadow-2xl max-w-md w-full p-6 space-y-4 animate-in fade-in zoom-in-95 duration-200">
            <div className="flex items-center justify-between border-b border-pink-100 pb-3">
              ...
              <button onClick={cancelPairing} className="text-pink-400 hover:text-pink-700 text-sm font-bold">✕</button>
            </div>
            <div className="text-center py-4 space-y-3">
              ...
              {pairingStatusMsg && <p className="text-xs text-pink-800 font-medium">{pairingStatusMsg}</p>}
            </div>
      ...
      <Card className="p-5 space-y-4">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 border-b border-pink-100 pb-4">
          <div className="flex items-center gap-2">
            <button onClick={() => setActiveTab('live_scans')} className={`text-xs sm:text-sm font-bold px-3.5 py-1.5 rounded-xl transition ${activeTab === 'live_scans' ? 'bg-pink-600 text-white shadow-sm' : 'text-pink-700 hover:bg-pink-50'}`}>
              ⏱️ Live Terminal Activity ({filteredScans.length})
            </button>
            <button onClick={() => setActiveTab('roster')} className={`text-xs sm:text-sm font-bold px-3.5 py-1.5 rounded-xl transition ${activeTab === 'roster' ? 'bg-pink-600 text-white shadow-sm' : 'text-pink-700 hover:bg-pink-50'}`}>
              👥 Employee Roster & RFID Keys ({filteredRoster.length})
            </button>
          </div>
          <input type="text" placeholder="Search employee, role, or ID..." value={filter} onChange={(e) => setFilter(e.target.value)} className="text-xs bg-pink-50 border border-pink-200 rounded-lg px-3 py-1.5 text-pink-950 focus:outline-none focus:ring-2 focus:ring-pink-400 w-full sm:w-64" />
        </div>
        ...
        {activeTab === 'live_scans' && (
          <Table>
            <THead>
              <TR header>
                <TH>Employee</TH><TH>Role & Terminal</TH><TH>Device Station</TH><TH>Time In</TH><TH>Time Out</TH><TH>Status</TH><TH className="text-right">Action</TH>
              </TR>
            </THead>
            <TBody>
                ...
                <TR key={row.id}>
                  <TD className="font-bold text-pink-950">
                    <div className="flex items-center gap-2.5">
                      <div className="w-8 h-8 rounded-full bg-pink-100 text-pink-800 font-bold text-xs flex items-center justify-center">{row.employee?.avatar_initials || '—'}</div>
                      <div>
                        <div className="text-sm">{row.employee?.full_name || 'Unknown'}</div>
                        <div className="text-[10px] text-pink-400 font-mono">{row.employee?.email || row.employee_id.slice(0, 8)}</div>
                      </div>
                    </div>
                  </TD>
                  ...
                  <TD><span className="font-mono text-[11px] bg-pink-50 border border-pink-200 px-2 py-0.5 rounded text-pink-800 font-semibold">{row.terminal.includes('ESP32') ? row.terminal : 'ESP32-GATE-01'}</span></TD>
                  <TD className="text-emerald-800 font-mono text-xs font-bold">{formatScan(row.time_in || row.last_scan)}</TD>
                  <TD className="text-rose-800 font-mono text-xs font-bold">{row.time_out ? formatScan(row.time_out) : <span className="text-pink-300 font-normal">--:--:--</span>}</TD>
                  <TD>
                  ...
                  <TD className="text-right">
                    <Button variant="secondary" onClick={() => startPairing(row.employee || { id: row.employee_id, full_name: 'Employee', role: 'Staff' })} className="text-[11px] py-1 px-2.5 flex items-center gap-1 ml-auto bg-pink-50 hover:bg-pink-100 text-pink-800 border-pink-200">
                      <Key size={12} className="text-pink-600" /><span>Pair Card</span>
                    </Button>
                  </TD>
                </TR>
              ))}
        ...
        {activeTab === 'roster' && (
          <Table>
            <THead>
              <TR header>
                <TH>Employee Name</TH><TH>Roster Role</TH><TH>Terminal Location</TH><TH>Registered Card UID</TH><TH className="text-right">Card Assignment</TH>
              </TR>
            </THead>
            <TBody>
              {filteredRoster.map((emp) => (
                <TR key={emp.id}>
                  <TD className="font-bold text-pink-950">
                    <div className="flex items-center gap-2.5">
                      <div className="w-8 h-8 rounded-full bg-pink-100 text-pink-800 font-bold text-xs flex items-center justify-center">{emp.avatar_initials || '👤'}</div>
                  ...
                  <TD>
                    {emp.rfid_uid ? (
                      <span className="font-mono text-xs bg-emerald-50 border border-emerald-300 text-emerald-800 px-2 py-0.5 rounded-lg font-bold">🔑 {emp.rfid_uid}</span>
                    ) : (
                      <span className="text-xs text-pink-400 italic">Unassigned</span>
                    )}
                  </TD>
                  <TD className="text-right">
                    <Button variant="secondary" onClick={() => startPairing(emp)} className="text-[11px] py-1 px-3 flex items-center gap-1.5 ml-auto bg-pink-50 hover:bg-pink-100 text-pink-800 border-pink-200">
                      <Key size={12} className="text-pink-600" /><span>{emp.rfid_uid ? 'Re-assign Card' : 'Assign RFID Card'}</span>
                    </Button>
                  </TD>
                </TR>
              ))}
