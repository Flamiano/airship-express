...
export default function LoadsPage() {
  ...
  const load = useCallback(async () => {
    try {
      const res = await fetch('/workforce-management-hr2/api/loads', {
  ...
  return (
    <DashboardLayout>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        {loads.map((ld) => (
          <Card key={ld.id} className="p-5 space-y-3 hover:border-pink-300 transition">
            <div className="flex items-center justify-between">
              <span className="text-xs font-mono font-bold text-pink-500">
                {ld.load_ref}
              </span>
              <div className="flex items-center gap-1.5">
                <Badge className={SHIFT_PRIORITY_BADGE[ld.priority]}>
                  {ld.priority} Priority
                </Badge>
                <Badge className={LOAD_STATUS_BADGE[ld.status]}>{ld.status}</Badge>
              </div>
            </div>
            ...
            <div className="flex items-center gap-2 text-xs text-pink-800 font-semibold">
              ...
              <span>{ld.destination}</span>
            </div>

            <div className="space-y-1.5 text-xs text-pink-800">
              <p className="flex items-center gap-2">
                <MapPin size={14} className="text-pink-500" />
                <span className="font-semibold">Pickup:</span> {ld.pickup_date}
              </p>
              <p className="flex items-center gap-2">
                <User size={14} className="text-pink-500" />
                <span className="font-semibold">Assigned:</span>{' '}
                {ld.driver?.full_name || 'Unassigned'}
              </p>
            </div>
              ...
              <div className="pt-2 border-t border-pink-100 grid grid-cols-2 gap-3">
                <div>
                  ...
                  <select
                    value={ld.driver_id ?? ''}
                    onChange={(e) =>
                      handlePatch(ld.id, { driver_id: e.target.value || null })
                    }
