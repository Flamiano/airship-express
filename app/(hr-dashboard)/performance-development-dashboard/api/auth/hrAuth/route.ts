...
export async function POST(request: Request) {
    ...
    if (lookupError || !employee) {
        console.error(
            "Performance employee lookup failed:",
            lookupError
        );
    ...
    if (!ALLOWED_ROLES.includes(employee.role)) {
        return NextResponse.json(
            {
                message:
                    "This account does not have access to the Performance & Development dashboard.",
            },
            { status: 403 }
        );
    }
    ...
    console.log(
        "Attempting Performance login with email:",
        employee.email
    );
