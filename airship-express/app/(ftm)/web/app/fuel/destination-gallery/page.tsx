...
const destinationEntries: DestinationEntry[] = [];

export default function DestinationGalleryPage() {
  const [search, setSearch] = useState("");
  const [selected, setSelected] = useState<DestinationEntry | null>(null);

  const filteredEntries = useMemo(() => {
    const q = search.toLowerCase();
    return destinationEntries.filter((entry) => {
      return (
  ...
  // Preserve layout; show placeholder entries when fuel data is absent.
        ...
        <header className="rounded-3xl border border-pink-100 bg-white p-6 shadow-sm shadow-pink-900/5 sm:p-8">
          <div className="flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
            <div>
              ...
              <h1 className="text-3xl font-extrabold tracking-tight text-slate-900">Destination gallery</h1>
              <p className="mt-2 max-w-2xl text-sm text-slate-600">
                Archive of courier destination images confirming parcel receipt and route completion at the end point.
              </p>
            </div>

            <div className="rounded-2xl bg-pink-50 px-4 py-3 text-sm font-semibold text-pink-700 ring-1 ring-pink-200">
              {hasData ? `${visibleEntries.length} destination proof entries` : "—"}
            </div>
          </div>
        </header>
        ...
        <section className="grid gap-5 md:grid-cols-2 xl:grid-cols-3">
          {visibleEntries.map((entry) => (
            <article key={entry.id} className="overflow-hidden rounded-3xl border border-pink-100 bg-white shadow-sm transition hover:-translate-y-0.5 hover:shadow-md">
              <button type="button" onClick={() => setSelected(entry)} className="block w-full text-left" aria-label={`View destination gallery for ${entry.vehicle}`}>
                <div className="relative aspect-[4/3] overflow-hidden bg-slate-100">
                  <img src={entry.image} alt={`${entry.vehicle} destination proof`} className="h-full w-full object-cover transition duration-300 hover:scale-[1.02]" />
              ...
              <div className="space-y-3 p-4">
                ...
                <button type="button" onClick={() => setSelected(entry)} className="inline-flex items-center gap-2 rounded-xl bg-[#b80049] px-3 py-2 text-sm font-semibold text-white transition hover:bg-[#96003b]">
                  View image
      ...
      {selected && (
        <div className="fixed inset-0 z-[1200] flex items-center justify-center bg-slate-950/70 p-4 backdrop-blur-sm">
          <div className="relative w-full max-w-4xl overflow-hidden rounded-3xl bg-white shadow-2xl">
            <button type="button" onClick={() => setSelected(null)} className="absolute right-4 top-4 z-10 flex h-10 w-10 items-center justify-center rounded-full bg-white/90 text-slate-700 shadow-sm transition hover:bg-white" aria-label="Close destination proof preview">
              <span className="material-symbols-outlined">close</span>
            </button>

            <div className="grid gap-0 lg:grid-cols-[1.5fr_0.9fr]">
              <div className="bg-slate-100">
                <img src={selected.image} alt={`${selected.vehicle} destination proof`} className="h-full max-h-[78vh] w-full object-contain" />
              </div>

              <div className="space-y-4 p-5 sm:p-6">
                <div className="flex items-center justify-between gap-3">
                  <div>
                    <p className="text-[10px] font-bold uppercase tracking-[0.2em] text-pink-700">Destination</p>
                    <h3 className="mt-1 text-2xl font-black text-slate-900">{selected.vehicle}</h3>
                  </div>
                  <span className="rounded-full bg-pink-50 px-2.5 py-1 text-xs font-bold text-pink-700 ring-1 ring-pink-100">{selected.id}</span>
                </div>
                ...
                <div className="rounded-2xl bg-slate-50 p-3 text-sm text-slate-700">{selected.note}</div>

                <a href={selected.image} target="_blank" rel="noreferrer" className="inline-flex w-full items-center justify-center gap-2 rounded-xl bg-[#b80049] px-4 py-3 text-sm font-semibold text-white transition hover:bg-[#96003b]">
                  Open original image
