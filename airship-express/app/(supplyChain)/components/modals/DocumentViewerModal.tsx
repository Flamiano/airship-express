...
export default function DocumentViewerModal({ isOpen, onClose, data }: DocumentViewerModalProps) {
    ...
    return (
        <Portal>
            <div className="fixed inset-0 z-[110] flex items-center justify-center p-3 sm:p-6 bg-slate-950/70 dark:bg-black/80 backdrop-blur-md animate-in fade-in duration-200" onClick={onClose}>
            <div
                className="relative w-full max-w-5xl h-[92vh] max-h-[900px] bg-[#f0f3f8] dark:bg-[#161722] border border-white/90 dark:border-white/[0.08] rounded-3xl shadow-[14px_14px_45px_rgba(166,175,195,0.45),-14px_-14px_45px_rgba(255,255,255,0.95)] dark:shadow-[16px_16px_50px_rgba(0,0,0,0.85)] overflow-hidden flex flex-col animate-in zoom-in-95 duration-200"
                onClick={(e) => e.stopPropagation()}
            >
                {/* Header */}
                <div className="flex items-center justify-between px-5 sm:px-6 py-4 border-b border-slate-200/60 dark:border-white/[0.06] bg-[#ebf0f7]/70 dark:bg-[#14151e]/60 shrink-0">
                    ...
                    <div className="flex items-center gap-1.5 sm:gap-2 shrink-0">
                        {!isPdf && resolvedUrl && (
                            <div className="hidden sm:flex items-center gap-1 bg-[#ebf0f7] dark:bg-[#14151e] p-1 rounded-2xl border border-white/80 dark:border-white/[0.06] shadow-[inset_1px_1px_2px_rgba(166,175,195,0.35),inset_-1px_-1px_2px_rgba(255,255,255,0.9)]">
                                <button type="button" onClick={handleZoomOut} className="p-1.5 text-slate-600 dark:text-slate-300 hover:text-pink-600 dark:hover:text-pink-400 rounded-xl hover:bg-white dark:hover:bg-slate-700 transition-all cursor-pointer" title="Zoom Out">
                                ...
                                <button type="button" onClick={handleZoomIn} className="p-1.5 text-slate-600 dark:text-slate-300 hover:text-pink-600 dark:hover:text-pink-400 rounded-xl hover:bg-white dark:hover:bg-slate-700 transition-all cursor-pointer" title="Zoom In">
                                    <i className="fas fa-search-plus text-xs" />
                                </button>
                                <button type="button" onClick={handleRotate} className="p-1.5 text-slate-600 dark:text-slate-300 hover:text-pink-600 dark:hover:text-pink-400 rounded-xl hover:bg-white dark:hover:bg-slate-700 transition-all cursor-pointer border-l border-slate-200 dark:border-slate-700 pl-2" title="Rotate 90°">
                                    <i className="fas fa-redo text-xs" />
                                </button>
                                <button type="button" onClick={handleResetZoom} className="p-1.5 text-slate-500 hover:text-slate-800 dark:text-slate-400 dark:hover:text-white rounded-xl hover:bg-white dark:hover:bg-slate-700 transition-all cursor-pointer text-[10px]" title="Reset">
                                    Reset
                                </button>
                            </div>
                        )}
                        ...
                        {resolvedUrl && (
                            <>
                                <a href={resolvedUrl} target="_blank" rel="noopener noreferrer">
                                    <AppButton type="button" variant="neutral" size="icon-sm" title="Open in new tab" aria-label="Open in new tab">
                                        <i className="fas fa-external-link-alt text-xs" />
                                    </AppButton>
                                </a>
                                <AppButton type="button" variant="neutral" size="icon-sm" onClick={handleDownload} disabled={isDownloading} title="Download Document directly to device" aria-label="Download Document">
                                    {isDownloading ? (<i className="fas fa-spinner fa-spin text-xs text-pink-500" />) : (<i className="fas fa-download text-xs" />)}
                                </AppButton>
                            </>
                        )}

                        <AppButton type="button" variant="neutral" size="icon-sm" onClick={onClose} title="Close preview" aria-label="Close preview" className="ml-1 hover:border-rose-300 hover:text-rose-600 dark:hover:border-rose-800 dark:hover:text-rose-400">
                ...
                <div className="flex-1 min-h-0 bg-slate-900/95 dark:bg-slate-950 relative overflow-auto flex items-center justify-center p-4">
                    ...
                    {hasError && !isLoading && (
                        <div className="flex flex-col items-center justify-center gap-2 text-center p-6 max-w-sm">
                            ...
                            {resolvedUrl && (
                                <a href={resolvedUrl} target="_blank" rel="noreferrer" className="mt-3 px-3.5 py-1.5 bg-pink-500 text-white rounded-xl text-xs font-semibold hover:bg-pink-600 transition-all shadow-xs">
                                    Open Link Directly
                                </a>
                            )}
                        </div>
                    ...
                    {!isLoading && !hasError && resolvedUrl && (
                        <EmbeddedDocViewer url={resolvedUrl} fileName={data.fileName} title={data.title} fileType={data.fileType} storagePath={data.storagePath} onDownload={handleDownload} zoom={zoom} rotation={rotation} minHeight="min-h-[460px] sm:min-h-[580px]" />
                    )}
                </div>

                {/* Footer Info Strip */}
                <div className="px-6 py-3 border-t border-slate-200/60 dark:border-white/[0.06] bg-[#ebf0f7]/70 dark:bg-[#14151e]/60 flex items-center justify-between flex-wrap gap-2 text-xs text-slate-600 dark:text-slate-400 shrink-0">
